...
function M.get_byte_offset_from_cursor()
    ...
    local buf_row = cursor[1] - 1 -- 0-based buffer row
    local col = cursor[2]
    -- In big file mode the buffer is a sliding viewport; add the viewport offset to
    -- recover the true file row the cursor is sitting on.
    local file_row = state.big_file and (buf_row + state.viewport_top_line) or buf_row
    local line_offset = file_row * BYTES_PER_LINE

    local line = vim.api.nvim_buf_get_lines(state.main_buf, buf_row, buf_row + 1, false)[1]
    if line then
...
function M.jump_to_offset(target)
    ...
    local file_row = math.floor(target / BYTES_PER_LINE)
    ...
    if state.big_file then
        -- Only reload the viewport when the target row falls outside the currently
        -- loaded buffer window; in-viewport jumps (e.g. one byte forward after an
        -- edit) are cheap: just reposition the cursor.
        local buf_count = vim.api.nvim_buf_line_count(state.main_buf)
        local in_viewport = file_row >= state.viewport_top_line
            and file_row < state.viewport_top_line + buf_count
        if not in_viewport then
            ...
            local top_line                    = math.max(0, file_row - math.floor(win_height / 2))
            local num_lines                   = math.min(total - top_line, win_height * 3)
            local lines                       = display.format_lines_for_viewport(top_line, num_lines)
            state.viewport_top_line           = top_line
        ...
        local buf_row = file_row - state.viewport_top_line + 1 -- 1-based
        if buf_row < 1 then buf_row = 1 end
        local buf_line_count = vim.api.nvim_buf_line_count(state.main_buf)
        if buf_row > buf_line_count then buf_row = buf_line_count end
        vim.api.nvim_win_set_cursor(state.main_win, { buf_row, col })
        return
    end

    vim.api.nvim_win_set_cursor(state.main_win, { file_row + 1, col })
end
