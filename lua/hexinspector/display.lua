...
function M.update_title()
    ...
    local title = " HexEditor │ " ..
    fname .. dirty_mark .. " │ " .. size_str .. " │ " .. tpl.name .. endian_mark .. big_mark .. " "
...
function M.refresh_display()
    ...
    if state.big_file then
        ...
        -- Buffer row is NOT the same as file row in streaming mode; add the viewport
        -- offset to recover the true file row the cursor is sitting on.
        local cur_file_row = (cursor_pos[1] - 1) + state.viewport_top_line
        local total = M.total_lines_for_file()
        local top_line = math.max(0, cur_file_row - math.floor(win_height / 2))
        -- Load 3× the window height so the user can scroll a full screen in either
        -- direction before the next reload; do NOT load all the way to EOF.
        ...
        -- After replacing the buffer the cursor is still at the old buffer row.
        -- Move it to the row that represents the same file position.
        local new_buf_row = cur_file_row - top_line + 1
        if new_buf_row < 1 then new_buf_row = 1 end
        if new_buf_row > #lines then new_buf_row = #lines end
        vim.api.nvim_win_set_cursor(state.main_win, { new_buf_row, cursor_pos[2] })
