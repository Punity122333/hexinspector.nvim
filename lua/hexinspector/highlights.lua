...
function M.highlight_cursor_byte(offset)
    ...
    -- In big file streaming mode the buffer is a viewport into the file.
    -- All Neovim buffer-row indices must be relative to the viewport start,
    -- i.e.  buf_row = file_row - viewport_top_line.
    local vp_off = state.big_file and state.viewport_top_line or 0
    ...
    if state.selecting and state.selection_start then
        ...
        for so = sel_s, sel_e do
            local sr = math.floor(so / BYTES_PER_LINE) - vp_off -- buffer row (0-based)
            local sb = so % BYTES_PER_LINE
            local sg = math.floor(sb / 4)
            local sc = HEX_START_COL + sb * 3 + sg
            if sr >= 0 and sr < line_count then
    ...
    local row = math.floor(offset / BYTES_PER_LINE) - vp_off -- buffer row (0-based)
    ...
    if offset + 4 <= state.file_size then
        for i = 0, 3 do
            local fo = offset + i
            local fr = math.floor(fo / BYTES_PER_LINE) - vp_off -- buffer row (0-based)
            local fb = fo % BYTES_PER_LINE
            local fg = math.floor(fb / 4)
            local fc = HEX_START_COL + fb * 3 + fg
            if fr >= 0 and fr < line_count and fo ~= offset then
